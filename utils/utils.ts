...
import {
  ...
  setLightness as polishedSetLightness,
} from "polished";
...
export function getAllColorVars(colors: typeof COLORS) {
  return Object.values(colors).flatMap((shades) =>
    Object.values(shades).map((shade) => getColorVar(shade))
  );
}

let timerId;

function waitForDOMHydration(tries = 0) {
  if (tries === 15) {
    return;
  }
  if (timerId) {
  ...
  const isReady = false; // check is ready
